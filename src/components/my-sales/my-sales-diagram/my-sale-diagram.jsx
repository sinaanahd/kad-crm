...
const MySaleDiagram = () => {
  const { all_payments, user } = useContext(DataContext);
  const p2e = (s) => s.replace(/[۰-۹]/g, (d) => "۰۱۲۳۴۵۶۷۸۹".indexOf(d));
  const make_data = (e) => {
    const my_payments = all_payments.filter(
      (p) => p.seller_name === user.fullname && p.is_payed
    );
    const null_obj = {
      name: "",
      amount: 0,
